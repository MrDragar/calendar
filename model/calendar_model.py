...
class CalendarModel:
    ...
    def set_data(self):
        ...
        self.month = self.localtime.tm_mon
        self.day = self.localtime.tm_mday
        self.first_wday = datetime.datetime(self.year, self.month, 1).weekday()
        self.current_year = self.year
        self.current_month = self.month
        self.current_data = True
        self.first_week = datetime.datetime(self.year, self.month, 1).isocalendar().week % \
                          datetime.datetime(self.year - 1, 12, 31).isocalendar().week
        self.days_count = monthrange(self.year, self.month)[1]
